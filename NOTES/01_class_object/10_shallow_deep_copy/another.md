...
2. **`operator=` 的陷阱**：很多人写了拷贝构造，却忘了写赋值运算符 (`operator=`)，导致 `obj1 = obj2` **(此时ojb已经存在而非初始化)** 时依然发生浅拷贝崩溃。
3. **自赋值检查**：在 `operator=` 中，必须检查 `if (this == &other)`，否则 `obj = obj` 会先删掉自己的数据，然后试图从已经删掉的数据里复制内容，导致错误。