 ...
 * @note 编译指令: g++ rtti_demo.cpp -o rtti_demo
 */
...
3. **对比 `static_cast**`：如果你确信 `gameObjects[0]` 一定是 `Player`，可以用 `static_cast`，它的开销更小（只是简单的指针计算），但如果搞错了，后果自负。在不可控的输入下，永远优先选 `dynamic_cast`。